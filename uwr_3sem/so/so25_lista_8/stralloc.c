...
static void *alloc_block(int8_t *data, uint8_t len) {
  ...
  while (!END_OF_CHUNK(data)) {
    if (BLOCK_USED(data)) {
      /* TODO: occupied block
       * Blok jest zajęty (wartość nagłówka dodatnia),
       * więc przechodzimy do następnego bloku używając MOVE_NEXT. */
      MOVE_NEXT(data);
    } else if (BLOCK_SIZE(data) == len) {
      /* TODO: free block of exact size
       *  Blok wolny ma dokładnie wymagany rozmiar.
       * Zmieniamy nagłówek z ujemnego na dodatni (oznaczając zajęty),
       * zwracamy wskaźnik na dane (pomijając bajt nagłówka). */
      BLOCK_HEADER(data) = len;
      result = CURRENT_PTR(data);
      break;
    } else if (BLOCK_SIZE(data) > len) {
      /* TODO: free block is too large
       *  Blok wolny jest za duży - dzielimy go (splitting).
       * Pierwszy fragment (len bajtów) oznaczamy jako zajęty,
       * drugi fragment (pozostała część) oznaczamy jako wolny.
       * Nagłówek drugiego fragmentu wpisujemy na pozycji cur_offset + len. */
      uint8_t old_size = BLOCK_SIZE(data);
      BLOCK_HEADER(data) = len;
      data[cur_offset + len] = -(old_size - len);
      result = CURRENT_PTR(data);
      break;
    } else if (!NEXT_BLOCK_FREE(data)) {
      /* TODO: next block is occupied or does not exists
       *  Blok jest za mały, ale następny jest zajęty lub
       * nie istnieje - nie możemy połączyć. Przechodzimy do następnego. */
      MOVE_NEXT(data);
    } else if (NEXT_BLOCK_SIZE(data) <= len - BLOCK_SIZE(data)) {
      /* TODO: merge two free blocks, but do not allocate
       *  Dwa wolne bloki razem są wciąż za małe.
       * Łączymy je (coalescing) w jeden większy wolny blok (ujemny nagłówek),
       * ale nie alokujemy. Nie przesuwamy cur_offset - ponownie sprawdzamy
       * ten połączony blok, może da się go połączyć z kolejnym. */
      uint8_t merged_size = BLOCK_SIZE(data) + NEXT_BLOCK_SIZE(data);
      BLOCK_HEADER(data) = -merged_size;
      /* don't move to next - recheck this merged block */
    } else {
      /* TODO: merge two free blocks and allocate with split
       * Dwa wolne bloki razem są wystarczające
       * Łączymy je, alokujemy wymaganą ilość, resztę zostawiamy jako wolny blok.
       * Pierwszy fragment (len) - zajęty, reszta - wolna */
      uint8_t merged_size = BLOCK_SIZE(data) + NEXT_BLOCK_SIZE(data);
      BLOCK_HEADER(data) = len;
      data[cur_offset + len] = -(merged_size - len);
      result = CURRENT_PTR(data);
      break;
    }
  }
...
static void strfree(char *str) {
   ...
   * Blok zajęty ma nagłówek dodatni, wolny - ujemny */
  sstr[-1] = -sstr[-1];
}
